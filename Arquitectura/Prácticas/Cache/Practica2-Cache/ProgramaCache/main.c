...
void parse_args(argc, argv) //argument count: contains the number of arguments passed to the program
  int argc;
  char **argv;
{
  ...
  for (i = 0; i < argc; i++)
    if (!strcmp(argv[i], "-h")) { //Compares strings
  ...
  /* open the trace file */
  traceFile = fopen(argv[arg_index], "r"); //Open a file and create a new stream for it.
...
void play_trace(inFile)
  FILE *inFile; //Inicializa un objeto del tipo file
{
